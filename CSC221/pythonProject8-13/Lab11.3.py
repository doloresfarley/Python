'''
11.13 LAB: Artwork label (classes/constructors)

LAB ACTIVITY
11.13.1: LAB: Artwork label (classes/constructors)
...
class Artist:
    ...
    def print_info(self):
        ...
        if self.death_year == -1 and self.birth_year == -1:
            print(f'Artist: {self.name} (unknown)')
        elif  self.death_year == -1 and self.birth_year != -1:
...
class Artwork:
    ...
    Define the Artwork class with a constructor to initialize an
     artwork's information and a print_info() method.
     The constructor should by default initialize
     the title to "unknown", the year created to -1,
